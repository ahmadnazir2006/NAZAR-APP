...
export default function Page() {
  ...
  // These hooks manage the camera and AI state
  ...
  // --- FIXED EXIT LOGIC ---
  // Instead of reloading the page, we just reset the app states
  const handleExit = () => {
    setIsHolding(false)
    stopCamera() // Shuts down camera stream
    if (typeof window !== "undefined") {
      window.speechSynthesis.cancel() // Stops AI from talking
    }
    // Note: We stay on the same page, but isActive becomes false, 
    // which shows the "Hold to See" button again.
  }
  ...
  // --- RESTORED ORIGINAL HOLD LOGIC ---
  const handleHoldStart = useCallback(() => {
    setIsHolding(true)
    startCamera()
    // Using your original logic for the start message
    const msg = lang === "ur" ? "کیمرہ آن ہو رہا ہے" : "Camera activating"
    speak(msg)
  }, [startCamera, lang])
  ...
  return (
    <main className="fixed inset-0 overflow-hidden bg-background select-none">
      ...
      <div className="absolute top-4 left-4 right-4 z-30 flex items-center justify-between gap-3">
        {/* Exit Button: Now calls handleExit which resets the hold state */}
        <button 
          onClick={handleExit}
          className="w-14 h-14 rounded-full border-2 border-primary/40 bg-background/80 flex items-center justify-center text-primary active:scale-90"
        >
        ...
        <button 
          onClick={cycleMode}
          className="flex-1 h-14 rounded-2xl border-b-4 border-primary bg-primary text-primary-foreground font-black text-lg shadow-lg active:border-b-0 active:translate-y-1 transition-all"
        >
        ...
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><circle cx="12" cy="12" r="3"/><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/></svg>
        </button>
      </div>

      <CameraPreview videoRef={videoRef} isActive={isActive} error={error} lang={lang} apiError={apiError} />

      {/* Visual Feedback - No changes here, keeps pointer-events-none */}
      {isProcessing && (
      ...
      {/* This button only shows when the camera is NOT active */}
      {!isActive && !error && (
        <TapToSeeButton 
          lang={lang} 
